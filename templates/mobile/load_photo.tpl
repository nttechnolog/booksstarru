...
$(document).ready(function(){
	Xajax = new AjaxUpload('upload', {
		...
		if (!(ext && /^(jpg|png|jpeg|gif|jpe)$/.test(ext))) {
			Box.Info('load_photo_er', lang_dd2f_no, lang_bad_format, 2000);
				return false;
			}
		...
		onComplete: function (file, response) {
			if(response == 'bad_format')
				...
				Box.Close('photo');
				$('#ava').html('<meta http-equiv="refresh" content="0; url=/" /><DIV style="BACKGROUND: url('+response+') no-repeat; background-size: 107px 107px" class="p1"></DIV>');
