...
var Albums = {
	...
	view: function(aid){
    viiBox.start();
	$.post('albums/ajax_view/'+aid, function(d){
	  viiBox.win('albums', d);
	});
  }
}
...
var fave = {
	add: function(fave_id){
		...
		$.post('/index.php?go=fave&act=add', {fave_id: fave_id}, function(data){
			...
			$('#text_add_fave').html('<img onmouseover="myhtml.title(\'106\', \'Удалить из закладок\', \'newBBlock1\')" id="newBBlock1106" width="16" src="/templates/Default/images/icons/fave_del.png" />');
			$('#addfave_load').hide();
		});
	},
	delet: function(fave_id){
		...
		$.post('/index.php?go=fave&act=delet', {fave_id: fave_id}, function(data){
			...
			$('#text_add_fave').html('<img onmouseover="myhtml.title(\'106\', \'Добавить в закладки\', \'newBBlock1\')" id="newBBlock1106" width="16" src="/templates/Default/images/icons/fave.png" />');
			$('#addfave_load').hide();
		});
	},
...
var notes = {
	send: function(){
		...
		if(title != 0){
			if(text != 0){
				...
				$.post('/index.php?go=notes&act=save', {title: title, text: text, news: news, mywall: mywall,chanr:chanr,form:form}, function(d){
					if(d == 'min_strlen')
    ...
    sendDuel: function(){
	    console.log('sendDuel');
        var chanr = $('#chanrStory').val();
        var aboutAutor = $('#aboutAutor').val();
        var nameStory = $('#nameStory').val();
        var story = $('#story').val();
        var hrefFullVersion = $('#hrefFullVersion').val();
        if(nameStory != 0){
            if(story != 0){
                $.post('/index.php?go=duel&act=sendDuel', {chanr: chanr, aboutAutor: aboutAutor, nameStory: nameStory, story: story, hrefFullVersion: hrefFullVersion}, function(d){
                        alert('заявка откправлена на обработку');
                    Box.Close();
                });
            } else {
                $("#story").val('');
                $("#story").css('background', '#ffefef').focus();
                setTimeout("$('#story').css('background', '#fff').focus()", 800);
            }
        } else {
            $("#nameStory").val('');
            $("#nameStory").css('background', '#ffefef').focus();
            setTimeout("$('#nameStory').css('background', '#fff').focus()", 800);
        }
    },
	editsave: function(note_id){
		...
		if(title != 0){
			if(text != 0){
				...
				$.post('/index.php?go=notes&act=editsave', {note_id: note_id, title: title, text: text,chanr:chanr,form:form}, function(d){
					if(d == 'min_strlen')
						Box.Info('msg_notes', lang_dd2f_no, lang_notes_no_text, 300);
					else
    ...
    previewDuel: function(){
        console.log('previewDuel');
        var title = $('#nameStory').val();
        var text = $('#story').val();
        if(title != 0){
            if(text != 0){
                Box.Page('/index.php?go=notes&act=preview', 'text='+text+'&title='+title, 'preview', 820, lang_notes_preview, lang_msg_close, 0, 0, 500, 1, 1, 1, 0, 0);
	...
	notes_add_like: function(rec_id, user_id, type){
		if($('#notes_like_cnt'+rec_id).text())
			var notes_like_cnt = parseInt($('#notes_like_cnt'+rec_id).text())+1;
		else {
			$('#public_likes_user_block'+rec_id).show();
			$('#update_like'+rec_id).val('1');
			var notes_like_cnt = 1;
		}
		
		$('#notes_like_cnt'+rec_id).html(notes_like_cnt).css('color', '#2f5879');
		$('#notes_active_ic'+rec_id).addClass('public_notes_like_yes');
		$('#notes_like_link'+rec_id).attr('onClick', 'notes.notes_remove_like('+rec_id+', '+user_id+', \''+type+'\')');
		$('#like_user'+user_id+'_'+rec_id).show();
		updateNum('#like_text_num'+rec_id, 1);
		
		if(type == 'uPages')
			$.post('/index.php?go=notes&act=like_yes', {rid: rec_id});
		else
			$.post('/index.php?go=groups&act=notes_like_yes', {rec_id: rec_id});
	},
	notes_remove_like: function(rec_id, user_id, type){
		var notes_like_cnt = parseInt($('#notes_like_cnt'+rec_id).text())-1;
		if(notes_like_cnt <= 0){
			var notes_like_cnt = '';
		...
		$('#notes_like_cnt'+rec_id).html(notes_like_cnt).css('color', '#95adc0');
		$('#notes_active_ic'+rec_id).removeClass('public_notes_like_yes');
		$('#notes_like_link'+rec_id).attr('onClick', 'notes.notes_add_like('+rec_id+', '+user_id+', \''+type+'\')');
		$('#Xlike_user'+user_id+'_'+rec_id).hide();
		$('#like_user'+user_id+'_'+rec_id).hide();
		updateNum('#like_text_num'+rec_id);

		if(type == 'uPages')
			$.post('/index.php?go=notes&act=like_no', {rid: rec_id});
		else
			$.post('/index.php?go=groups&act=notes_like_remove', {rec_id: rec_id});
	},
	notes_like_users_five: function(rec_id, type){		
		$('.public_likes_user_block').hide();
		if(!ge('like_cache_block'+rec_id) && $('#notes_like_cnt'+rec_id).text() && $('#update_like'+rec_id).val() == 0){
			if(type == 'uPages'){
				$.post('/index.php?go=notes&act=liked_users', {rid: rec_id}, function(data){
					$('#likes_users'+rec_id).html(data+'<span id="like_cache_block'+rec_id+'"></span>');
					$('#public_likes_user_block'+rec_id).show();
				});
			} else {
				$.post('/index.php?go=groups&act=notes_like_users_five', {rec_id: rec_id}, function(data){
					$('#likes_users'+rec_id).html(data+'<span id="like_cache_block'+rec_id+'"></span>');
					$('#public_likes_user_block'+rec_id).show();
				});
			}
	...
	notes_like_users_five_hide: function(){
		$('.public_likes_user_block').hide();
	},
	notes_all_liked_users: function(rid, page_num, liked_num){
		$('.public_likes_user_block').hide();
		if(page_num)
			page = '&page='+page_num;
		else {
		...
		Box.Page('/index.php?go=groups&act=all_liked_users', 'rid='+rid+'&liked_num='+liked_num+page, 'all_liked_users_'+rid+page_num, 525, lang_notes_liked_users, lang_msg_close, 0, 0, 345, 1, 1, 1, 0, 1);
	}
}
...
var subscriptions = {
	add: function(for_user_id){
		...
		$.post('/index.php?go=subscriptions&act=add', {for_user_id: for_user_id}, function(d){
			...
			$('#text_add_subscription').html('<img onmouseover="myhtml.title(\'105\', \'Отписаться от обновлений\', \'newBBlock1\')" id="newBBlock1105" width="16" src="/templates/Default/images/icons/power.png" />');
	...
	del: function(del_user_id){
		...
		$.post('/index.php?go=subscriptions&act=del', {del_user_id: del_user_id}, function(){
			$('#addsubscription_load').hide();
			$('#text_add_subscription').html('<img onmouseover="myhtml.title(\'105\', \'Подписаться на обновления\', \'newBBlock1\')" id="newBBlock1105" width="16" src="/templates/Default/images/icons/reload.png" />');
...
var gSearch = {
	...
	chSpisok:function(){
	  $("#onspisok").toggleClass("html_checked");
	 
	  var isSpisok=$("#onspisok").hasClass("html_checked");
	  if(isSpisok==true){
	    
	     document.location.href="/?go=search&type=3&query=&spisok=true";
	  }
	  else{
	 
	    document.location.href="/?go=search&type=3&query=";
	  }
	},
    chEditor:function(){
        $("#oneditor").toggleClass("html_checked");

        var isEditor=$("#oneditor").hasClass("html_checked");
        if(isEditor==true){

            document.location.href="/?go=search&type=7&query=&editor=true";
        }
        else{
...
var wall = {
	...
	attach_addgraffiti:function(id){
		$.post('/index.php?go=graffiti&id='+id+'&act=add',{id:id},function(body){
		$('html').css({'overflow':'hidden'});$('body').append('<div id="newbox_miniature"><div class="miniature_box"><div class="miniature_pos" style="width: 600px;"><div class="miniature_title fl_l">Граффити на стену</div><a class="cursor_pointer fl_r" onclick="Profile.miniatureClose()">Закрыть</a><div class="clear"></div>'+body+'</div></div></div>');
        });
    },
	attach_addnotes: function(id){
		viiBox.start();
		$.post('/ajax_notes',{id:id}, function(d){
		viiBox.win('notes', d);
		});
	},
	...
	attach_mapsBox: function(id){
		viiBox.start();
		$.post('/ajax_mapsBox'+id, function(d){
		viiBox.win('maps', d);
		});
	},
	tell: function(id){
...
var settings = {
	...
	saveChanr: function(){
		var user_chanr = $('#user_chanr').val();
		var user_forms = $('#user_forms').val();
		butloading('saveNewChanr', 69, 'disabled');
		$.post('/index.php?go=settings&act=savechanr', {user_chanr: user_chanr, user_forms: user_forms}, function(data){
					$('#ok_chanr').show();
					butloading('saveNewChanr', 69, 'enabled', 'Изменить');
		});
	...
	addblacklist: function(bad_user_id){
		...
		$.post('/index.php?go=settings&act=addblacklist', {bad_user_id: bad_user_id}, function(){
			...
			$('#text_add_blacklist').html('<img onmouseover="myhtml.title(\'103\', \'Разблокировать пользователя\', \'newBBlock1\')" id="newBBlock1103" width="16" src="/templates/Default/images/icons/over_off.png" />');
	...
	delblacklist: function(bad_user_id, type){
		...
		$.post('/index.php?go=settings&act=delblacklist', {bad_user_id: bad_user_id}, function(){
			if(type){
				...
				$('#text_add_blacklist').html('<img onmouseover="myhtml.title(\'103\', \'Заблокировать пользователя\', \'newBBlock1\')" id="newBBlock1103" width="16" src="/templates/Default/images/icons/power.png" />');
				$('#addblacklist_load').hide();
			} else {
...
var gifts = {
	...
	ajax_g: function(uid){
    viiBox.start();
	$.post('/ajax_gifts'+uid, function(d){
	  viiBox.win('gifts', d);
	});
  }
}
...
var groups = {
	...
	addadmin: function(id){
		var new_admin_id = $('#new_admin_id').val().replace('http://mypages.su/u', '');
...
//LIKAN
var likan = {
  wall_post: function(id){
    viiBox.start();
	$.post('/index.php?go=wall&act=wall_post&id='+id, function(d){
	  viiBox.win('wall_post', d);
	});
  },
  notifications: function(){
    viiBox.start();
	$.post('/index.php?go=notifications&act=notifications', function(d){
	  viiBox.win('notifications', d);
	});
  },
  newphoto: function(){
    viiBox.start();
	$.post('/index.php?go=likan&act=newphoto', function(d){
	  viiBox.win('newphoto', d);
	});
  },
  photo: function(id){
    viiBox.start();
	$.post('/index.php?go=likan&act=photo&id='+id, function(d){
	  viiBox.win('ourphoto', d);
	});
  },
  iesire: function(){
    viiBox.start();
	$.post('/index.php?go=likan&act=iesire', function(d){
	  viiBox.win('iesire', d);
	});
  }
}
...
var maps = {
	m_box: function(id){
		var title = $('#title_maps').val();
		if(title != 0){
				butloading('mapsBox_sending', 74, 'disabled');
				$.post('/index.php?go=mapsbox&act=send&id='+id, {id: id, title: title}, function(){
					butloading('mapsBox_sending', 74, 'enabled', 'Прикрепить местоположение');
				});
		}
	}
}
